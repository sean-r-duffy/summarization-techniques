...
train_dataset = SummarizationDataset(ds["train"])
train_loader = DataLoader(train_dataset, batch_size= 32, shuffle= True)

val_dataset = SummarizationDataset(ds["validation"])
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader, desc= f"Training Epoch: {epoch}"):
        ...
        loss = loss_fn(predictions, cosine_labels)
    ...
    # Validation
    scoring_model.eval() # set to eval so no updates
    val_loss = 0
    with torch.no_grad():
